...
task("dockerBuild") {
    ...
    doLast {
        ...
        project.exec {
            workingDir(dir)
            commandLine("docker", "build", "-t", "pfcjeong/guam-letter:${project.version}", ".")
            commandLine("docker", "build", "-t", "pfcjeong/guam-letter:latest", ".")
        }
    }
}
...
task("dockerPush") {
    ...
    doLast {
        project.exec {
            commandLine("docker", "push", "pfcjeong/guam-letter:${project.version}")
            commandLine("docker", "push", "pfcjeong/guam-letter:latest")
        }
    }
}
